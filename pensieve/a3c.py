...
GAMMA = 0.99
ENTROPY_EPS = 1e-6
#moved to multi_agent as feed_dict()
...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        ...
        #self.entropy=INITIAL_ENTROPY_WEIGHT
        # Create the actor network
        self.inputs, self.out = self.create_actor_network()
        #self.dnn_mode=opt.dnn_mode
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            split_4 = tflearn.conv_1d(inputs[:, 4:5, :len(opt.bitrate)], 128, 4, activation='relu')
    ...
    def train(self, inputs, acts, act_grad_weights, entropy_weight, lr_rate):
    ...
    def apply_gradients(self, actor_gradients,lr_rate):
        temp_store={ i: d for i, d in zip(self.actor_gradients, actor_gradients)}
        temp_store[self.lr_rate]=lr_rate
        return self.sess.run(self.optimize, feed_dict=temp_store)
        #return self.sess.run(self.optimize, feed_dict={
        #    i: d for i, d in zip(self.actor_gradients, actor_gradients) , self.lr_rate : lr_rate })
...
class CriticNetwork(object):
    ...
    def apply_gradients(self, critic_gradients,lr_rate):
        temp_store={i: d for i, d in zip(self.critic_gradients, critic_gradients)}
        temp_store[self.lr_rate]=lr_rate
        return self.sess.run(self.optimize, feed_dict=temp_store) 
        #return self.sess.run(self.optimize, feed_dict={
        #    i: d for i, d in zip(self.critic_gradients, critic_gradients) , self.lr_rate : lr_rate 
        #})
